<script lang="ts">
	...
	const handleCreateProject = async () => {
		if (files.length === 0 || !project_name) {
			alert('No file uploaded or file name is empty!');
			return;
		}
		...
		try {
			...
			localStorage.setItem('currentFile', project_name);
			goto(`/DebtSheet/file_overview/${CustomError.project_id}`);

			const result = response;
			console.log('file uploaded successfully!(from import.svelte)', result);
			files = [];
			fetchFiles();
			isloading = false;
			modal.isModalOpen = false;
		} catch (err) {
			console.error('Error uploading file:', CustomError.error);
...
<Card
	class="relative flex min-w-[40%] max-w-[200px] flex-col gap-4 rounded-3xl border-none p-4 shadow-none"
>
	{#if isloading}
	...
	{:else}<div class="">
			...
			>
			<p class=" text-[.8em]">
				Your transaction sheet has be uploaded and Will be processed for further Fixes
			</p>
		</div>

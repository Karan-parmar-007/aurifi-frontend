<script lang="ts">
	...
	async function handleSaveRule() {
		...
		// Prepare payload
		const payload = {
			user_id: '68131e7bb9f75b267c7386a9', // Hardcoded for now
		...
		try {
			...
			const response = await fetch(
				`${VITE_API_URL}/rules_book_debt/add_rule?update=true`,
				{
					method: 'POST',
					headers: {
	...
	const fetchDatatypeMapping = async () => {
		try {
			isLoading = true;
			const response = await fetch(
				`${VITE_API_URL}/project/get_datatype_mapping/${projectId}`
			);

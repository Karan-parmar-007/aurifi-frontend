<script lang="ts">
	...
	const downloadFile = (filepath: string, name: string) => {
		const downloadUrl = `${VITE_API_URL}/project/download_file/${filepath}`;
	...
	const moveToArchive = async () => {
		...
		try {
			...
			const response = await fetch(
				`${VITE_API_URL}/archive_debt_sheet/send_project_to_archive`,
				{
					method: 'POST',
					headers: {
						'Content-Type': 'application/json'
								...
								<TableBodyCell class=" rounded-md bg-transparent"
									><span class=" flex items-center justify-end gap-4">
										...
										{:else}<button
												...
												onclick={() => downloadFile(items.base_file_path, items.name)}
												class="button-group"
											>
												<svg
													width="23"

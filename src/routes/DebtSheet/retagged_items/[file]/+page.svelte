<script lang="ts">
	...
	const finalizeRules = async () => {
		try {
			isloading = true;
			const response = await fetch(
				`${VITE_API_URL}/dataset/finalize_temp_versions`,
				{
					method: 'POST',
					headers: {
	...
	const handleDownloadCombined = async () => {
		if (combinedFilePath) {
			await downloadFile(combinedFilePath, 'combined_final.xlsx');
		} else {
			console.error('No combined file path available');
		}
	};

	const handleDownloadOption = async (option: string, tagName: string) => {
		const response = await fetch(
			`${VITE_API_URL}/dataset/fetch_data_after_applied_rules?project_id=${project_id}`
		);
		if (!response.ok) {
		...
		switch (option) {
			case 'Download All': {
				const version = versions.find(
					(v: any) => v.tag_name.toLowerCase() === tagName.toLowerCase()
				);
				if (version?.file_path) {
					await downloadFile(version.file_path, `${tagName}_current.xlsx`);
				}

				const addedFile = rowsAddedFiles.find(
					(f: any) => f.tag_name.toLowerCase() === tagName.toLowerCase()
				);
				if (addedFile?.file_location) {
					await downloadFile(addedFile.file_location, `${tagName}_inserted.xlsx`);
				}
				...
				if (removedFile?.file_location) {
					await downloadFile(removedFile.file_location, `${tagName}_ejected.xlsx`);
				}
				break;
			}
			case 'Current': {
				...
				if (version?.file_path) {
					...
					console.error(`No current file found for tag: ${tagName}`);
				}
				break;
			}
			case 'Inserted': {
				...
				if (addedFile?.file_location) {
					...
					console.error(`No inserted file found for tag: ${tagName}`);
				}
				break;
			}
			case 'Ejected': {
				...
				if (removedFile?.file_location) {
					...
					console.error(`No ejected file found for tag: ${tagName}`);
				}
				break;
			}
			default:
	...
	const redoTagging = async () => {
		try {
			isloading = true;
			const response = await fetch(
				`${VITE_API_URL}/dataset/disable_rule_addition_for_project`,
				{
					method: 'POST',
						...
						<div class="flex h-full w-[30%] items-center justify-between">
							...
							<span class="mr-4 flex items-center justify-around gap-6">
								...
								<div class="dropdown-trigger">
									...
									<Dropdown class="w-40">
										<DropdownItem
											onclick={() => handleDownloadOption('Download All', file.tag_name)}
										>
											Download All
										</DropdownItem>
										<DropdownItem onclick={() => handleDownloadOption('Current', file.tag_name)}>
											Current
										</DropdownItem>
										<DropdownItem onclick={() => handleDownloadOption('Inserted', file.tag_name)}>
											Inserted
										</DropdownItem>
										<DropdownItem onclick={() => handleDownloadOption('Ejected', file.tag_name)}>
											Ejected
										</DropdownItem>
									</Dropdown>
								</div>

<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			// Check for errors in any columns
			hasErrors =
				dateColumns.some((col) => col.error) ||
...
{#if NumberFormatModal.isNumberFormatModalOpen && selectedColumn}
	<NumberFormatModalForm
		data={numericColumns.filter((col) => col.column_name === selectedColumn)}
		v_id={version_id}
		onUpdate={(status) => handleUpdate(selectedColumn, status)}
		isTransaction={true}
	/>
...
{#if CurrencyFormatModal.isCurrencyFormatModalOpen && selectedColumn}
	<CurrencyFormatModalForm
		data={currencyColumns.filter((col) => col.column_name === selectedColumn)}
		v_id={version_id}
		onUpdate={(status) => handleUpdate(selectedColumn, status)}
		isTransaction={true}
	/>
						...
						<TableBody>
							{#each dateColumns as column}
								<TableBodyRow>
									...
									<TableBodyCell></TableBodyCell>
									<TableBodyCell>
										{#if fixStatus[column.column_name]?.success}

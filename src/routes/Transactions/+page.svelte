<script lang="ts">
	...
	const fetchFiles = async () => {
		try {
			...
			files = result.transactions.map((transaction) => ({
				_id: transaction._id,
	...
	const downloadFile = (filePath) => {
		const downloadUrl = `${VITE_API_URL}/project/download_file/${encodeURIComponent(
			filePath
		)}`;
		const a = document.createElement('a');
		a.href = downloadUrl;
		// Extract filename from base_file_path for download
			...
			<div class="h-full w-full">
				{#if showArchiveConfirm}
					<div class="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm">
						<div
							class="pក System: w-96 rounded-lg

<script lang="ts">
	...
	let editingRuleData = $state(null);
	let fileRules = $state([]);
	let deletingRuleIndex = $state(null);
	...
	$effect(() => {
		if (applying_rules && fileRules.length === 0) {
			// Add some initial sample rules
			fileRules = [
				[
					{
						column: 'State',

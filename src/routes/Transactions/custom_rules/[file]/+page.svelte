<script lang="ts">
	...
	import AdditionalRule from '$lib/components/AdditionalRule.svelte';
	import { page } from '$app/state';
	import BreadCrumb from '$lib/components/BreadCrumb.svelte';
	import RuleModalForm from '$lib/components/RuleModalForm.svelte';
...
<div class="flex min-h-[100vh] w-full flex-col">
	...
	<div class="h-full px-20">
		<BreadCrumb />
		{#if errorMessage}
			<p class="text-red-500 px-6 py-2">{errorMessage}</p>
		{/if}
						...
						<div class="flex w-full flex-col items-end gap-2 pe-20">
							{#each versions as version, versionIndex}
								<div class="flex w-full items-center justify-end gap-2">
									...
									>
										{version.rule_applied.map((ruleGroup) => formatRuleGroup(ruleGroup)).join(' | ')}
									</span>
									<span
									...
									<span class="flex items-center justify-evenly gap-4 px-2">
										...
										<button aria-label="delete button" class="cursor-pointer">
											<svg
												width="14"
												height="17"
												viewBox="0 0 14 17"

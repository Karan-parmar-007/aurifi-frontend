<script lang="ts">
	...
	async function fetchData() {
		try {
			const response = await fetch('http://localhost:5000/api/v1/admin/get_system_columns');
	...
	function openAddModal() {
		editingColumn = null; // Reset for adding new column
		systemColumnModal.isSystemColumnModalOpen = true;
	}

	function openEditModal(column) {
		editingColumn = column; // Set the column to edit
		systemColumnModal.isSystemColumnModalOpen = true;
	}
</script>
...
<div class="flex w-full flex-col items-center justify-center">
	<div class="h-[80%] w-[92%] py-10">
		...
		<AdminAddColumnForm {editingColumn} {fetchData} />
	</div>
</div>
